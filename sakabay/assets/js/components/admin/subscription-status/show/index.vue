    ...
    <div v-else>
      ...
      >
        ...
        <div class="col-6 justify-content-end">
          <a
            class="float-right"
            :href="'/admin/subscription-status/edit/' + subscriptionStatusId"
          >
      ...
      <div class="register-card mt-3 w-100 h-100">
        ...
        <div class="row mb-2">
          <div class="col-6">
            <span class="fontHelveticaOblique fontSize18">{{ this.subscriptionStatus.name.toUpperCase() }}</span>
          </div>
          <div class="col-6">
            <span class="fontHelveticaOblique fontSize18">{{ this.subscriptionStatus.code }}</span>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    async created() {
      if (this.subscriptionStatusId) {
        this.loading = true;
        return axios.get('/api/admin/subscription-status/' + this.subscriptionStatusId)
          .then(response => {
