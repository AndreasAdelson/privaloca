          ...
          <div class="row my-3">
            <div class="col-6 offset-3">
              <button
                ...
                @click="$validateForm()"
              >
                {{ this.subscriptionStatusId ? this.$t('commons.edit') : this.$t('commons.create') }}
              </button>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        loading: true,
        API_URL: '/api/admin/subscription-status' + (this.subscriptionStatusId ? `/${this.subscriptionStatusId}` : ''),
        formFields: {
          name: null,
          code: null,
        },
    ...
    created() {
      ...
      return Promise.all(promises).then(res => {
        let subscriptionStatus = res[0].data;
        this.$removeFieldsNotInForm(subscriptionStatus, Object.keys(this.formFields));
        this.$setEditForm(subscriptionStatus);
        this.loading = false;
      }).catch(e => {
        this.$handleError(e);
        this.loading = false;
