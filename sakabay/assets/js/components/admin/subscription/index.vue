<template>
  <div class="container-fluid skb-body">
        ...
        >
          <template #cell(actions)="data">
            <b-button-group>
              <a
                v-if="canRead"
                :href="'/admin/subscription/show/' + data.value "
              >
                <b-button><font-awesome-icon :icon="['fas', 'eye']" /></b-button>
              </a>
              <a
                v-if="canEdit"
                :href="'/admin/subscription/edit/' + data.value "
                class="mx-1"
              >
    ...
    <confirm-modal
      ...
      @confirm-modal-yes="$deleteEntity('/api/admin/subscriptions/')"
    />
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        table: {
          field: [
            { key: 'name', label: this.$t('subscription.fields.name'), sortable: true, thClass: 'tableitem' },
            { key: 'code', label: this.$t('subscription.fields.code'), sortable: true, thClass: 'tableitem' },
            { key: 'price', label: this.$t('subscription.fields.price'), sortable: true, thClass: 'tableitem' },
            { key: 'advantages', label: this.$t('subscription.fields.advantages'), thClass: 'tableitem' },
    ...
    methods: {
      refreshData() {
        ...
        return axios.get('/api/admin/subscriptions', {
          ...
          let items = _.map(response.data, subscription => _.assign(subscription, {
