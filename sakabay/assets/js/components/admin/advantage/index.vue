<template>
  <div class="container-fluid skb-body">
        ...
        >
          <template #cell(actions)="data">
            <b-button-group>
              <a
                v-if="canRead"
                :href="'/admin/advantage/show/' + data.value "
              >
                <b-button><font-awesome-icon :icon="['fas', 'eye']" /></b-button>
              </a>
              <a
                v-if="canEdit"
                :href="'/admin/advantage/edit/' + data.value "
                class="mx-1"
              >
    ...
    <confirm-modal
      ...
      @confirm-modal-yes="$deleteEntity('/api/admin/advantages/')"
    />
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        table: {
          field: [
            { key: 'message', label: this.$t('advantage.fields.message'), sortable: true, thClass: 'tableitem' },
            { key: 'code', label: this.$t('advantage.fields.code'), sortable: true, thClass: 'tableitem' },
            { key: 'priority', label: this.$t('advantage.fields.priority'), sortable: true, thClass: 'tableitem' },
    ...
    methods: {
      refreshData() {
        this.loading = true;
        return axios.get('/api/admin/advantages', {
          params: {
          ...
          let items = _.map(response.data, advantage => _.assign(advantage, {
            message: advantage.message,
            priority: advantage.priority,
            code: advantage.code,
            actions: advantage.id,
          }));
